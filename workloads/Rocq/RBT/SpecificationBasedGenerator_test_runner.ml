...
let gen_between lo hi _ =
  if Z.leb (Big_int_Z.mult_int_big_int 2 Big_int_Z.unit_big_int) (Z.sub hi lo)
  then bindGen
         (choose producerGen chooseZ ((Z.add lo Big_int_Z.unit_big_int),
           (Z.sub hi Big_int_Z.unit_big_int))) (fun x ->
         ret (Obj.magic monadGen) (Some x))
  else ret (Obj.magic monadGen) None

(** val genSizedSuchThatred_black_bst :
    color -> int -> Big_int_Z.big_int -> Big_int_Z.big_int -> tree0
    genSizedSuchThat **)

let genSizedSuchThatred_black_bst c_ h_ lo_ hi_ =
  let aux_arb =
    let rec aux_arb init_size size0 c_0 h_0 lo_0 hi_0 =
      (fun fO fS n -> if n=0 then fO () else fS (n-1))
        (fun _ ->
        backtrack (((Stdlib.Int.succ 0),
          (thunkGen (fun _ ->
            (fun fO fS n -> if n=0 then fO () else fS (n-1))
              ...
              (fun n0 ->
              (fun fO fS n -> if n=0 then fO () else fS (n-1))
                (fun _ -> returnGen (Some E))
                (fun _ -> returnGen None)
                n0)
              h_0))) :: (((Stdlib.Int.succ 0),
            ...
            (fun fO fS n -> if n=0 then fO () else fS (n-1))
              ...
              | B ->
                bindOpt (Obj.magic producerGen)
                  (genSuchThatOfBounded (Obj.magic gen_between lo_0 hi_0))
                  (fun x ->
                  bindGen (genOfGenSized genSizedColor) (fun c1 ->
                    bindOpt (Obj.magic producerGen)
                      (aux_arb init_size size' c1 h lo_0 x) (fun l ->
                      bindGen (genOfGenSized genSizedColor) (fun c2 ->
                        bindOpt (Obj.magic producerGen)
                          (aux_arb init_size size' c2 h x hi_0) (fun r ->
                          bindGen (genOfGenSized genZSized) (fun y ->
                            returnGen (Some (T (B, l, x, y, r))))))))))
              h_0))) :: (((Stdlib.Int.succ size'),
          (thunkGen (fun _ ->
            match c_0 with
            | R ->
              bindOpt (Obj.magic producerGen)
                ...
                (fun x ->
                bindOpt (Obj.magic producerGen)
                  (aux_arb init_size size' B h_0 lo_0 x) (fun l ->
                  bindOpt (Obj.magic producerGen)
                    (aux_arb init_size size' B h_0 x hi_0) (fun r ->
                    bindGen (genOfGenSized genZSized) (fun y ->
                      returnGen (Some (T (R, l, x, y, r)))))))
            | B -> returnGen None))) :: []))))
        size0
    in aux_arb
  in
  (fun size0 -> aux_arb size0 size0 c_ h_ lo_ hi_)

(** val gRbt : tree0 option g **)

let gRbt =
  genSizedSuchThatred_black_bst B (Stdlib.Int.succ (Stdlib.Int.succ
    (Stdlib.Int.succ 0))) Big_int_Z.zero_big_int
    (Big_int_Z.mult_int_big_int 2 (Big_int_Z.mult_int_big_int 2
    ((fun x -> Big_int_Z.succ_big_int (Big_int_Z.mult_int_big_int 2 x))
    (Big_int_Z.mult_int_big_int 2 (Big_int_Z.mult_int_big_int 2
    ((fun x -> Big_int_Z.succ_big_int (Big_int_Z.mult_int_big_int 2 x))
    Big_int_Z.unit_big_int)))))) (Stdlib.Int.succ (Stdlib.Int.succ
    (Stdlib.Int.succ (Stdlib.Int.succ (Stdlib.Int.succ (Stdlib.Int.succ
    (Stdlib.Int.succ (Stdlib.Int.succ (Stdlib.Int.succ (Stdlib.Int.succ
    0))))))))))
...
let test_prop_InsertValid =
  forAllMaybe testChecker showTree gRbt (fun t0 ->
    forAll testChecker showZ (genOfGenSized genZSized) (fun k ->
...
let test_prop_DeleteValid =
  forAllMaybe testChecker showTree gRbt (fun t0 ->
    forAll (checkable_opt testBool) showZ (genOfGenSized genZSized) (fun k ->
