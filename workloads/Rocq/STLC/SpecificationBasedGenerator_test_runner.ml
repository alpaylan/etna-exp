...
(** val dec_type0 : typ -> typ -> dec **)

let rec dec_type0 t0 x =
  match t0 with
  | TBool -> (match x with
              | TBool -> true
              | TFun (_, _) -> false)
  | TFun (t1, t2) ->
    (match x with
     | TBool -> false
     | TFun (t3, t4) -> if dec_type0 t1 t3 then dec_type0 t2 t4 else false)
...
(** val genSizedSuchThatbind : ctx -> typ -> int genSizedSuchThat **)

let genSizedSuchThatbind env_ tau_ =
  let aux_arb =
    let rec aux_arb init_size size0 env_0 tau_0 =
      (fun fO fS n -> if n=0 then fO () else fS (n-1))
        (fun _ ->
        backtrack (((Stdlib.Int.succ 0),
          (thunkGen (fun _ ->
            ...
            | [] -> returnGen None
            | tau :: _ ->
              (match dec_decOpt (dec_type0 tau_0 tau) init_size with
               | Some res_b ->
                 if res_b then returnGen (Some 0) else returnGen None
               | None -> returnGen None)))) :: (((Stdlib.Int.succ 0),
          ...
          (thunkGen (fun _ ->
            ...
            | tau :: _ ->
              (match dec_decOpt (dec_type0 tau_0 tau) init_size with
               | Some res_b ->
                 if res_b then returnGen (Some 0) else returnGen None
               | None -> returnGen None)))) :: (((Stdlib.Int.succ size'),
          (thunkGen (fun _ ->
            match env_0 with
            | [] -> returnGen None
            | _ :: env ->
              bindOpt (Obj.magic producerGen)
                (aux_arb init_size size' env tau_0) (fun x ->
                returnGen (Some (Stdlib.Int.succ x)))))) :: [])))
        size0
    in aux_arb
  in
  (fun size0 -> aux_arb size0 size0 env_ tau_)

(** val genSizedSuchThattyping : ctx -> typ -> expr genSizedSuchThat **)

let genSizedSuchThattyping env_ tau_ =
  let aux_arb =
    let rec aux_arb init_size size0 env_0 tau_0 =
      (fun fO fS n -> if n=0 then fO () else fS (n-1))
        (fun _ ->
          ...
          (thunkGen (fun _ ->
            bindOpt (Obj.magic producerGen)
              (genSuchThatOfBounded
                (Obj.magic genSizedSuchThatbind env_0 tau_0)) (fun x ->
              returnGen (Some (Var x)))))) :: (((Stdlib.Int.succ 0),
          (thunkGen (fun _ ->
            match tau_0 with
            | TBool ->
              bindGen (genOfGenSized genBoolSized) (fun b ->
                returnGen (Some (Bool b)))
            | TFun (_, _) -> returnGen None))) :: (((Stdlib.Int.succ 0),
          (thunkGen (fun _ -> returnGen None))) :: []))))
        (fun size' ->
        backtrack (((Stdlib.Int.succ 0),
          (thunkGen (fun _ ->
          ...
          (thunkGen (fun _ ->
            ...
            | TBool ->
              bindGen (genOfGenSized genBoolSized) (fun b ->
                returnGen (Some (Bool b)))
            | TFun (_, _) -> returnGen None))) :: (((Stdlib.Int.succ size'),
          (thunkGen (fun _ ->
            match tau_0 with
            | TBool -> returnGen None
            | TFun (t1, t2) ->
              bindOpt (Obj.magic producerGen)
                (aux_arb init_size size' (t1 :: env_0) t2) (fun e ->
                returnGen (Some (Abs (t1, e))))))) :: (((Stdlib.Int.succ
          size'),
          (thunkGen (fun _ ->
            bindGen (genOfGenSized genSizedTyp0) (fun t1 ->
              bindOpt (Obj.magic producerGen)
                (aux_arb init_size size' env_0 (TFun (t1, tau_0))) (fun e1 ->
                bindOpt (Obj.magic producerGen)
                  (aux_arb init_size size' env_0 t1) (fun e2 ->
...
(** val gTypedExpr : ctx -> typ -> int -> expr option g **)

let gTypedExpr =
  genSizedSuchThattyping
...
let gSized =
  bind (Obj.magic monadGen) (genOfGenSized (Obj.magic genSizedTyp0))
    (fun typ0 ->
    gTypedExpr [] typ0 (Stdlib.Int.succ (Stdlib.Int.succ (Stdlib.Int.succ
      (Stdlib.Int.succ (Stdlib.Int.succ 0))))))

(** val test_prop_SinglePreserve : checker **)

let test_prop_SinglePreserve =
  forAllMaybe (checkable_opt testBool) showExpr gSized prop_SinglePreserve

(** val test_prop_MultiPreserve : checker **)

let test_prop_MultiPreserve =
  forAllMaybe (checkable_opt testBool) showExpr gSized prop_MultiPreserve
          ...
          (withTime (fun _ ->
            quickCheckWith testChecker
              (updMaxDiscard
                ...
                num_tests) test_prop_SinglePreserve)))
        ('}'::('|'::(']'::[])))))
        ...
        (showTimedResult showResult
          (withTime (fun _ ->
            quickCheckWith testChecker
              (updMaxDiscard
                (updMaxSuccess (updAnalysis stdArgs true) num_tests)
                num_tests) test_prop_MultiPreserve))) ('}'::('|'::(']'::[])))))
