...
let elems_ pG def l =
  let n0 = length l in
  bind pG.super (choose pG chooseNat (0, (subn n0 (Stdlib.Int.succ 0))))
...
let rec dec_type t0 x =
  ...
  | TFun (t1, t2) ->
    (match x with
     | TBool -> false
     | TFun (t3, t4) -> if dec_type t1 t3 then dec_type t2 t4 else false)
...
| Var n0 ->
  append ('('::('V'::('a'::('r'::(' '::[]))))) (append (showNat n0) (')'::[]))
| Bool b ->
  append ('('::('B'::('o'::('o'::('l'::(' '::[]))))))
    (append (sexp b) (')'::[]))
| Abs (t0, e') ->
  append ('('::('A'::('b'::('s'::(' '::[])))))
    (append (showTyp t0) (append (' '::[]) (append (showExpr e') (')'::[]))))
| App (e1, e2) ->
  append ('('::('A'::('p'::('p'::(' '::[])))))
...
let rec getTyp ctx0 = function
| Var n0 -> nth_error ctx0 n0
| Bool _ -> Some TBool
| Abs (t0, e0) ->
  bind (Obj.magic monad_option) (getTyp (t0 :: ctx0) e0) (fun t' -> Some
    (TFun (t0, t')))
| App (e1, e2) ->
  bind (Obj.magic monad_option) (getTyp ctx0 e1) (fun t1' ->
    match t1' with
    | TBool -> None
    | TFun (t11, t12) ->
      bind (Obj.magic monad_option) (getTyp ctx0 e2) (fun t2 ->
        if dec_type t11 t2 then Some t12 else None))

(** val typeCheck : ctx -> expr -> typ -> bool **)

let typeCheck ctx0 e t0 =
  match getTyp ctx0 e with
  | Some t' -> if dec_type t0 t' then true else false
  | None -> false
...
let shift d ex =
  ...
  | Var n0 ->
    if Nat.ltb n0 (Z.to_nat c)
    then Var n0
    else Var (Z.to_nat (Z.add (Z.of_nat n0) d))
  | Bool b -> Bool b
  | Abs (t0, e0) -> Abs (t0, (go (Z.add Big_int_Z.unit_big_int c) e0))
  | App (e1, e2) -> App ((go c e1), (go c e2))
  in go Big_int_Z.zero_big_int ex

(** val subst : int -> expr -> expr -> expr **)

let rec subst n0 s = function
| Var m -> if Nat.eqb m n0 then s else Var m
| Bool b -> Bool b
| Abs (t0, e0) ->
  Abs (t0,
    (subst (add n0 (Stdlib.Int.succ 0)) (shift Big_int_Z.unit_big_int s) e0))
| App (e1, e2) -> App ((subst n0 s e1), (subst n0 s e2))

(** val substTop : expr -> expr -> expr **)

let substTop s e =
  subst 0 (shift Big_int_Z.unit_big_int s) e

(** val fromMaybe : 'a1 -> 'a1 option -> 'a1 **)

let fromMaybe a = function
| Some value -> value
| None -> a

(** val pstep : expr -> expr option **)

let rec pstep = function
| Abs (t0, e0) ->
  bind (Obj.magic monad_option) (pstep e0) (fun e' -> Some (Abs (t0, e')))
| App (e1, e2) ->
  (match e1 with
   | Var _ ->
     (match pstep e1 with
      | Some e0 ->
        let e1' = fromMaybe e1 (Some e0) in
        let e2' = fromMaybe e2 (pstep e2) in Some (App (e1', e2'))
      | None ->
        (match pstep e2 with
         | Some e0 ->
           let e1' = fromMaybe e1 None in
           let e2' = fromMaybe e2 (Some e0) in Some (App (e1', e2'))
         | None -> None))
   | Bool _ ->
     (match pstep e1 with
      | Some e0 ->
      ...
      | None ->
        (match pstep e2 with
         ...
         | None -> None))
   | Abs (_, e3) ->
     let e1' = fromMaybe e3 (pstep e3) in
     let e2' = fromMaybe e2 (pstep e2) in Some (substTop e2' e1')
   | App (_, _) ->
     (match pstep e1 with
      | Some e0 ->
        let e1' = fromMaybe e1 (Some e0) in
...
let mtypeCheck e t0 =
  match e with
  | Some e' -> typeCheck [] e' t0
  | None -> true

(** val prop_SinglePreserve : expr -> bool option **)

let prop_SinglePreserve e =
  implies0 impliesBO (isJust (mt e))
    (bind (Obj.magic monad_option) (Obj.magic mt e) (fun t' -> Some
      (mtypeCheck (pstep e) t')))

(** val prop_MultiPreserve : expr -> bool option **)

let prop_MultiPreserve e =
  implies0 impliesBO (isJust (mt e))
    (bind (Obj.magic monad_option) (Obj.magic mt e) (fun t' -> Some
      (mtypeCheck
        (multistep (Stdlib.Int.succ (Stdlib.Int.succ (Stdlib.Int.succ
          (Stdlib.Int.succ (Stdlib.Int.succ (Stdlib.Int.succ (Stdlib.Int.succ
          (Stdlib.Int.succ (Stdlib.Int.succ (Stdlib.Int.succ (Stdlib.Int.succ
          (Stdlib.Int.succ (Stdlib.Int.succ (Stdlib.Int.succ (Stdlib.Int.succ
          (Stdlib.Int.succ (Stdlib.Int.succ (Stdlib.Int.succ (Stdlib.Int.succ
          ...
          (Stdlib.Int.succ (Stdlib.Int.succ (Stdlib.Int.succ (Stdlib.Int.succ
          (Stdlib.Int.succ (Stdlib.Int.succ (Stdlib.Int.succ (Stdlib.Int.succ
          (Stdlib.Int.succ 0)))))))))))))))))))))))))))))))))))))))) pstep e)
        t')))

(** val genSizedTyp : typ genSized **)

let rec genSizedTyp size =
  (fun fO fS n -> if n=0 then fO () else fS (n-1))
    ...
    freq_ (returnGen TBool) (((Stdlib.Int.succ 0),
      ...
      (thunkGen (fun _ ->
        bindGen (genSizedTyp size') (fun p0 ->
          bindGen (genSizedTyp size') (fun p1 -> returnGen (TFun (p0, p1))))))) :: [])))
    size

(** val fuzzyTyp : typ fuzzy **)

let rec fuzzyTyp = function
| TBool ->
  bindGen (genOfGenSized genSizedTyp) (fun param0 ->
    bindGen (genOfGenSized genSizedTyp) (fun param1 ->
      returnGen (TFun (param0, param1))))
| TFun (p0, p1) ->
  oneOf_ (Obj.magic producerGen)
    (bindGen (fuzzyTyp p1) (fun fuzzed -> returnGen (TFun (p0, fuzzed))))
    ((thunkGen (fun _ ->
       bindGen (fuzzyTyp p1) (fun fuzzed -> returnGen (TFun (p0, fuzzed))))) :: (
    (thunkGen (fun _ -> returnGen p1)) :: ((thunkGen (fun _ ->
                                             bindGen (fuzzyTyp p0)
                                               (fun fuzzed ->
                                               returnGen (TFun (fuzzed, p1))))) :: (
    (thunkGen (fun _ -> returnGen p0)) :: ((thunkGen (fun _ ->
                                             returnGen TBool)) :: [])))))
...
let rec fuzzynat x' =
  (fun fO fS n -> if n=0 then fO () else fS (n-1))
    (fun _ ->
    bindGen (genOfGenSized genNatSized) (fun param0 ->
      returnGen (Stdlib.Int.succ param0)))
    (fun p0 ->
    oneOf_ (Obj.magic producerGen)
      (bindGen (fuzzynat p0) (fun fuzzed ->
        returnGen (Stdlib.Int.succ fuzzed)))
      ((thunkGen (fun _ ->
         bindGen (fuzzynat p0) (fun fuzzed ->
           returnGen (Stdlib.Int.succ fuzzed)))) :: ((thunkGen (fun _ ->
                                                       returnGen p0)) :: (
      (thunkGen (fun _ -> returnGen 0)) :: []))))
    x'

(** val fuzzybool : bool fuzzy **)

let fuzzybool = function
| true -> returnGen false
| false -> returnGen true

(** val genSizedExpr : expr genSized **)

let rec genSizedExpr size =
  (fun fO fS n -> if n=0 then fO () else fS (n-1))
    (fun _ ->
    oneOf_ (Obj.magic producerGen)
      (bindGen (genOfGenSized genNatSized) (fun p0 -> returnGen (Var p0)))
      ((thunkGen (fun _ ->
         bindGen (genOfGenSized genNatSized) (fun p0 -> returnGen (Var p0)))) :: (
      (thunkGen (fun _ ->
        bindGen (genOfGenSized genBoolSized) (fun p0 -> returnGen (Bool p0)))) :: [])))
    (fun size' ->
    freq_
      (bindGen (genOfGenSized genNatSized) (fun p0 -> returnGen (Var p0)))
      (((Stdlib.Int.succ 0),
      (thunkGen (fun _ ->
        bindGen (genOfGenSized genNatSized) (fun p0 -> returnGen (Var p0))))) :: (((Stdlib.Int.succ
      0),
      (thunkGen (fun _ ->
        bindGen (genOfGenSized genBoolSized) (fun p0 -> returnGen (Bool p0))))) :: (((Stdlib.Int.succ
      size'),
      (thunkGen (fun _ ->
        bindGen (genOfGenSized genSizedTyp) (fun p0 ->
          bindGen (genSizedExpr size') (fun p1 -> returnGen (Abs (p0, p1))))))) :: (((Stdlib.Int.succ
      size'),
      (thunkGen (fun _ ->
        bindGen (genSizedExpr size') (fun p0 ->
          bindGen (genSizedExpr size') (fun p1 -> returnGen (App (p0, p1))))))) :: [])))))
    size

(** val fuzzyExpr : expr fuzzy **)

let rec fuzzyExpr = function
| Var p0 -> bindGen (fuzzynat p0) (fun fuzzed -> returnGen (Var fuzzed))
| Bool p0 -> bindGen (fuzzybool p0) (fun fuzzed -> returnGen (Bool fuzzed))
| Abs (p0, p1) ->
  oneOf_ (Obj.magic producerGen)
    (bindGen (fuzzyExpr p1) (fun fuzzed -> returnGen (Abs (p0, fuzzed))))
    ((thunkGen (fun _ ->
       bindGen (fuzzyExpr p1) (fun fuzzed -> returnGen (Abs (p0, fuzzed))))) :: (
    (thunkGen (fun _ -> returnGen p1)) :: ((thunkGen (fun _ ->
                                             bindGen (fuzzyTyp p0)
                                               (fun fuzzed ->
                                               returnGen (Abs (fuzzed, p1))))) :: [])))
| App (p0, p1) ->
  oneOf_ (Obj.magic producerGen)
    (bindGen (fuzzyExpr p1) (fun fuzzed -> returnGen (App (p0, fuzzed))))
    ((thunkGen (fun _ ->
       bindGen (fuzzyExpr p1) (fun fuzzed -> returnGen (App (p0, fuzzed))))) :: (
    (thunkGen (fun _ -> returnGen p1)) :: ((thunkGen (fun _ ->
                                             bindGen (fuzzyExpr p0)
                                               (fun fuzzed ->
                                               returnGen (App (fuzzed, p1))))) :: (
    (thunkGen (fun _ -> returnGen p0)) :: []))))
...
let test_prop_SinglePreserve =
  prop_SinglePreserve

(** val test_prop_SinglePreserve_fuzzer : unit -> result **)

let test_prop_SinglePreserve_fuzzer _ =
  fuzzLoopWith
    (updMaxDiscard (updMaxSuccess (updAnalysis stdArgs true) num_tests)
      num_tests) (genOfGenSized genSizedExpr) fuzzyExpr showExpr
    test_prop_SinglePreserve

(** val test_prop_MultiPreserve : expr -> bool option **)

let test_prop_MultiPreserve =
  prop_MultiPreserve

(** val test_prop_MultiPreserve_fuzzer : unit -> result **)

let test_prop_MultiPreserve_fuzzer _ =
  fuzzLoopWith
    (updMaxDiscard (updMaxSuccess (updAnalysis stdArgs true) num_tests)
      num_tests) (genOfGenSized genSizedExpr) fuzzyExpr showExpr
...
let qctest_test_prop_SinglePreserve _ =
  print_extracted_coq_string
    (append ('['::('|'::('{'::[])))
      (append
        (showTimedResult showResult
          (withTime test_prop_SinglePreserve_fuzzer)) ('}'::('|'::(']'::[])))))

(** val qctest_test_prop_MultiPreserve : unit -> unit **)

let qctest_test_prop_MultiPreserve _ =
  print_extracted_coq_string
    (append ('['::('|'::('{'::[])))
      (append
        (showTimedResult showResult (withTime test_prop_MultiPreserve_fuzzer))
        ('}'::('|'::(']'::[])))))
