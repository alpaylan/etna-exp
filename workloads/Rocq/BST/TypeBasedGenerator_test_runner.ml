...
(** val whenFail' : 'a1 checkable -> (unit -> char list) -> 'a1 -> checker **)

let whenFail' h str =
  callback0 h (PostFinalFailure (Counterexample, (fun _ _ ->
    trace (append (str ()) nl) 0)))
...
let test_prop_InsertValid =
  forAll testChecker showTree (arbitrary (genOfGenSized genSizedTree))
    (fun t0 ->
...
let test_prop_UnionValid =
  forAll testChecker showTree (arbitrary (genOfGenSized genSizedTree))
    (fun t1 ->
    forAll (checkable_opt testBool) showTree
      (arbitrary (genOfGenSized genSizedTree)) (fun t2 ->
...
let test_prop_UnionModel =
  forAll testChecker showTree (arbitrary (genOfGenSized genSizedTree))
    (fun t0 ->
    forAll (checkable_opt testBool) showTree
      (arbitrary (genOfGenSized genSizedTree)) (fun t' ->
...
let test_prop_DeleteDelete =
  forAllShrink testChecker showTree (arbitrary (genOfGenSized genSizedTree))
    (shrink0 shrinkTree) (fun t0 ->
    forAllShrink testChecker showNat (arbitrary (genOfGenSized genNatSized))
      (shrink0 shrinkNat) (fun k ->
      forAllShrink testChecker showNat
        (arbitrary (genOfGenSized genNatSized)) (shrink0 shrinkNat)
        (fun k' ->
        whenFail' (checkable_opt testBool) (fun _ ->
          show0
            (showPair
              (showPair
                (showPair
                  (showPair (showPair (showPair showTree showNat) showNat)
                    showTree) showTree) showTree) showTree) ((((((t0, k),
            k'), (delete k t0)), (delete k' t0)), (delete k (delete k' t0))),
            (delete k' (delete k t0)))) (prop_DeleteDelete t0 k k'))))
...
let test_prop_UnionUnionIdem =
  forAll (checkable_opt testBool) showTree
    (arbitrary (genOfGenSized genSizedTree)) prop_UnionUnionIdem
