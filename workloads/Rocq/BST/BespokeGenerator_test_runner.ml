...
let rec props' t0 n0 pf shrinker x =
  (fun fO fS n -> if n=0 then fO () else fS (n-1))
    (fun _ -> MkRose ((t0 (pf x)), (lazy [])))
    (fun n' -> MkRose ((t0 (pf x)), (lazy
...
let shrinking h shrinker x0 pf =
  fmap (functor_Monad (Obj.magic monadGen)) (fun x ->
    joinRose (fmapRose (fun q -> q) x))
    (Obj.magic promote (props h pf shrinker x0))
...
let forAllShrink x h gen0 shrinker pf =
  bindGen gen0 (fun x0 ->
    shrinking testChecker shrinker x0 (fun x' ->
      printTestCase x (append (h x') newline) (pf x')))
...
| T (p0, p1, p2, p3) ->
  app
    ...
    (app (app (map (fun shrunk -> T (p0, shrunk, p2, p3)) (shrinkNat p1)) [])
      (app
        (app (map (fun shrunk -> T (p0, p1, shrunk, p3)) (shrinkNat p2)) [])
        (app
          (app (p3 :: [])
...
let rec gen_bst s lo hi =
  (fun fO fS n -> if n=0 then fO () else fS (n-1))
    ...
    freq_ (ret (Obj.magic monadGen) E) (((Stdlib.Int.succ 0),
      (ret (Obj.magic monadGen) E)) :: (((if dec_class_dec
                                               (Nat.coq_Decidable_le_nat
                                                 (Stdlib.Int.succ
                                                 (sub hi lo))
                                                 (Stdlib.Int.succ
                                                 (Stdlib.Int.succ 0)))
      ...
      (bind (Obj.magic monadGen)
        ...
        (fun k ->
        bind (Obj.magic monadGen) (genOfGenSized (Obj.magic genNatSized))
          (fun v ->
...
let test_prop_InsertPost =
  forAllShrink testChecker showTree bespoke shrinkTree (fun t0 ->
    forAll testChecker showNat (genOfGenSized genNatSized) (fun k ->
      forAll testChecker showNat (genOfGenSized genNatSized) (fun k' ->
        forAll (checkable_opt testBool) showNat (genOfGenSized genNatSized)
