...
(** val promote : 'a1 g rose -> 'a1 rose g **)

let promote m n0 r =
  fmapRose (fun g0 -> run g0 n0 r) m
...
let mapProp x f prop =
  fmap (functor_Monad (Obj.magic monadGen)) f (checker0 x prop)
...
let props h pf shrinker x =
  props' h (Stdlib.Int.succ (Stdlib.Int.succ (Stdlib.Int.succ
    (Stdlib.Int.succ (Stdlib.Int.succ (Stdlib.Int.succ (Stdlib.Int.succ
    (Stdlib.Int.succ (Stdlib.Int.succ (Stdlib.Int.succ (Stdlib.Int.succ
    (Stdlib.Int.succ (Stdlib.Int.succ (Stdlib.Int.succ (Stdlib.Int.succ
    (Stdlib.Int.succ (Stdlib.Int.succ (Stdlib.Int.succ (Stdlib.Int.succ
...
let shrinking h shrinker x0 pf =
  fmap (functor_Monad (Obj.magic monadGen)) (fun x ->
    joinRose (fmapRose unProp x)) (Obj.magic promote (props h pf shrinker x0))
...
let forAll x h gen0 pf =
  bindGen gen0 (fun x0 ->
    printTestCase x (append (show0 h x0) newline) (pf x0))

(** val forAllShrink :
    'a2 checkable -> 'a1 show -> 'a1 g -> ('a1 -> 'a1 list) -> ('a1 -> 'a2)
    -> checker **)
...
let quickCheckWith x a p =
  ...
  | Some p0 ->
    ...
    test { maxSuccessTests = a.maxSuccess; maxDiscardedTests = a.maxDiscard;
      ...
      stDoAnalysis = a.analysis } (run (checker0 x p))
  | None ->
...
(** val shrinkNatAux : int -> int list **)

let rec shrinkNatAux x =
  (fun fO fS n -> if n=0 then fO () else fS (n-1))
    (fun _ -> [])
    (fun n0 ->
    (Nat.div (Stdlib.Int.succ n0) (Stdlib.Int.succ (Stdlib.Int.succ 0))) :: 
    (shrinkNatAux
      (Nat.div (Stdlib.Int.succ n0) (Stdlib.Int.succ (Stdlib.Int.succ 0)))))
    x

(** val shrinkNat : int shrink **)
...
(** val prop_UnionUnionAssoc : tree0 -> tree0 -> tree0 -> bool option **)

let prop_UnionUnionAssoc t1 t2 t3 =
  implies0 impliesBB ((&&) ((&&) (isBST t1) (isBST t2)) (isBST t3))
    (tree_eqb (union (union t1 t2) t3) (union t1 (union t2 t3)))

(** val shrinkTree : tree0 shrink **)

let rec shrinkTree = function
| E -> []
| T (p0, p1, p2, p3) ->
  app
    (app (p0 :: [])
      (app (map (fun shrunk -> T (shrunk, p1, p2, p3)) (shrinkTree p0)) []))
    (app
      (app
        (map (fun shrunk -> T (p0, shrunk, p2, p3)) (shrink0 shrinkNat p1))
        [])
      (app
        (app
          (map (fun shrunk -> T (p0, p1, shrunk, p3)) (shrink0 shrinkNat p2))
          [])
        (app
          (app (p3 :: [])
            (app (map (fun shrunk -> T (p0, p1, p2, shrunk)) (shrinkTree p3))
              [])) [])))

(** val gen_bst : int -> int -> int -> tree0 g **)
...
let test_prop_InsertValid =
  forAll testChecker showTree bespoke (fun t0 ->
    forAll testChecker showNat (arbitrary (genOfGenSized genNatSized))
      (fun k ->
      forAll (checkable_opt testBool) showNat
        (arbitrary (genOfGenSized genNatSized)) (fun v ->
        prop_InsertValid t0 k v)))
...
let test_prop_DeleteValid =
  forAll testChecker showTree bespoke (fun t0 ->
    forAll (checkable_opt testBool) showNat
      (arbitrary (genOfGenSized genNatSized)) (fun k -> prop_DeleteValid t0 k))

(** val test_prop_UnionValid : checker **)

let test_prop_UnionValid =
  forAll testChecker showTree bespoke (fun t1 ->
    forAll (checkable_opt testBool) showTree bespoke (fun t2 ->
      prop_UnionValid t1 t2))

(** val test_prop_InsertPost : checker **)

let test_prop_InsertPost =
  forAllShrink testChecker showTree bespoke (shrink0 shrinkTree) (fun t0 ->
    forAll testChecker showNat (arbitrary (genOfGenSized genNatSized))
      (fun k ->
      forAll testChecker showNat (arbitrary (genOfGenSized genNatSized))
        (fun k' ->
...
let test_prop_DeletePost =
  forAll testChecker showTree bespoke (fun t0 ->
    forAll testChecker showNat (arbitrary (genOfGenSized genNatSized))
      (fun k ->
      forAll (checkable_opt testBool) showNat
        (arbitrary (genOfGenSized genNatSized)) (fun k' ->
...
let test_prop_UnionPost =
  forAll testChecker showTree bespoke (fun t0 ->
    forAll testChecker showTree bespoke (fun t' ->
      forAll (checkable_opt testBool) showNat
        (arbitrary (genOfGenSized genNatSized)) (fun k ->
        prop_UnionPost t0 t' k)))
...
let test_prop_DeleteModel =
  forAll testChecker showTree bespoke (fun t0 ->
    forAll (checkable_opt testBool) showNat
      (arbitrary (genOfGenSized genNatSized)) (fun k -> prop_DeleteModel t0 k))

(** val test_prop_UnionModel : checker **)

let test_prop_UnionModel =
  forAll testChecker showTree bespoke (fun t0 ->
    forAll (checkable_opt testBool) showTree bespoke (fun t' ->
  ...
  forAll testChecker showTree bespoke (fun t0 ->
    forAll testChecker showTree bespoke (fun t' ->
      forAll testChecker showNat (arbitrary (genOfGenSized genNatSized))
        ...
        forAll (checkable_opt testBool) showNat
          (arbitrary (genOfGenSized genNatSized)) (fun v ->
          prop_UnionDeleteInsert t0 t' k v))))

(** val test_prop_UnionUnionIdem : checker **)

let test_prop_UnionUnionIdem =
  forAll (checkable_opt testBool) showTree bespoke prop_UnionUnionIdem

(** val test_prop_UnionUnionAssoc : checker **)

let test_prop_UnionUnionAssoc =
  forAll testChecker showTree bespoke (fun t1 ->
    forAll testChecker showTree bespoke (fun t2 ->
      forAll (checkable_opt testBool) showTree bespoke (fun t3 ->
...
(** val qctest_test_prop_UnionUnionAssoc : unit -> unit **)

let qctest_test_prop_UnionUnionAssoc _ =
  print_extracted_coq_string
    (append ('['::('|'::('{'::[])))
      (append
        (show0 (showTimedResult showResult)
          (withTime (fun _ ->
            quickCheckWith testChecker
              (updMaxDiscard
                (updMaxSuccess (updAnalysis stdArgs true) num_tests)
                num_tests) test_prop_UnionUnionAssoc)))
        ('}'::('|'::(']'::[])))))
...
fun test_name ->
  let test_map = [
    ...
    ("UnionUnionAssoc", qctest_test_prop_UnionUnionAssoc)
  ] in
