...
let arbitrarySizeST genSizedSuchThat0 =
  genSizedSuchThat0
...
let forAllMaybe x h gen0 pf =
  bindGen gen0 (fun mx ->
    match mx with
    | Some x0 -> printTestCase x (append (show0 h x0) newline) (pf x0)
...
let dec_decOpt h _ =
  if dec0 h then Some true else Some false
          ...
          (thunkGen (fun _ ->
            (fun fO fS n -> if n=0 then fO () else fS (n-1))
              ...
              (fun fO fS n -> if n=0 then fO () else fS (n-1))
                ...
                (fun m ->
                match decOpt0
                        (dec_decOpt (dec_class_dec (decidable_le_nat lo_0 m)))
                        init_size with
                | Some res_b ->
                  if res_b
